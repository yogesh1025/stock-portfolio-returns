...
def simulate(df: pd.DataFrame, initial: float, reinvest: bool):
    ...
    points = []
    splits = []
    div_history = []  # (timestamp, dividend_per_share) for trailing-12-month yield
...
def api_simulate():
    ...
    try:
        points, summary, splits = simulate(df, initial, reinvest)
